...
"""
    ...
    def create_resource_file(self, dest_res):
        ...
        for script in ScriptList:
            ...
            for res in script.ScriptCode.cres:
                ih.puts(external_addr, struct.pack('B', res))
                external_addr += 1  # sizeof(unsigned short)
            ...
            for script_res in script.ScriptResourceCodeList:
                ...
                for res in script_res.cres:
                    ih.puts(external_addr, struct.pack('H', res))
    ...
    def create_resource_map(self, file_name):
        print('\n----- Building C map file ------')
        h_file_name = "%s%s" % (file_name, self.HEADER_EXT)
        h_file = open(h_file_name, 'wt')
        ...
        c_file.write(self.C_FILE_DATA_BEGIN.format(h_file_name.replace(os.sep, '/')))
...
def generate(resource_path, dest_res, dest_c_map):
    start_build = time.time()
    res_map = FlashResourceGen()
    res_map.generatre_resources(resource_path)
    res_map.create_resource_file(dest_res)
...
if __name__ == '__main__':
    ...
    generate(args.resource_path, args.destination, args.map)
