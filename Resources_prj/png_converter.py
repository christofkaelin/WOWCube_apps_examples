...
def png_2_rgb565(png_file_name):
    if Image is None:
        print("Python module 'Pillow' is not installed")
        exit(1)
...
def estimate_size(path):
    ...
    for root, dir, files in os.walk(path):
        for file in fnmatch.filter(files, '*.png'):
            rgb, rgb_rle = png_2_cubios_picture('%s\%s' % (root, file))
...
def convert_picture(pic_path, rle):
    ...
    fl.write('const uint16_t {}[{}] = {{\n'.format(arr_name, len(final_arr)))
    for s in textwrap.wrap(', '.join(format(x, '#06x') for x in final_arr), 100):
        fl.write('    {}\n'.format(s))
    fl.write('};\n')
    fl.close()
